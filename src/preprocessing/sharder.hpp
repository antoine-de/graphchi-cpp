...
#define SHARDER_BUFSIZE (64 * 1024 * 1024)
    ...
    public:
        ...
        void preprocessing_add_edge(vid_t from, vid_t to, EdgeDataType val, bool input_value=false) {
            ...
            edge_with_value<EdgeDataType> edge(from, to, val);
#ifdef DYNAMICEDATA
            edge.is_chivec_value = input_value;
            if (edge.src == last_added_edge.src && edge.dst == last_added_edge.dst) {
                edge.valindex = last_added_edge.valindex + 1;
            }
            last_added_edge = edge;
#endif
            curshovel_buffer[curshovel_idx++] = edge;
    ...
    protected:
        ...
        void createnextshard() {
            ...
            logstream(LOG_INFO) << "Adding new shard from " << this_interval_start << " to " << (shardnum == nshards - 1 ? max_vertex_id : prevvid) << std::endl;
